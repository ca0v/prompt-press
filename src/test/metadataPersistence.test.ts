...
runner.describe('Metadata Persistence Tests', () => {
    ...
    const initial = `---\nartifact: ui-ux\nphase: requirement\ndepends-on: []\nreferences: [@ref:game-board.req, @ref:faction.req]\nversion: 1.0.0\nlast-updated: 2025-12-17\n---\n\n# UI/UX - Requirements\n\n## Overview\nText`;
    ...
    Assert.ok(/references:\s*\[.*["']@ref:game-board\.req["'].*["']@ref:faction\.req["'].*\]/.test(updatedText), 'references preserved');
    });
    ...
    it('should preserve non-empty depends-on list on save', async () => {
        ...
        Assert.ok(/references:\s*\[.*["']@ref:game-board\.req["'].*\]/.test(updatedText), 'references preserved');
    });
});
